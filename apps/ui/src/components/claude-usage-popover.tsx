...
export function ClaudeUsagePopover() {
    const { claudeRefreshInterval } = useAppStore();
    ...
    const fetchUsage = async (isAutoRefresh = false) => {
        ...
        try {
            const api = getElectronAPI();
            if (!api.claude) {
                throw new Error("Either your key is incorrect or Claude Usage API is not available");
            }
    ...
    }, [open, hasKey, claudeRefreshInterval]);
    ...
    const ProgressBar = ({ percentage, colorClass }: { percentage: number; colorClass: string }) => (
        <div className="h-2 w-full bg-secondary/50 rounded-full overflow-hidden">
            <div className={cn("h-full transition-all duration-500", colorClass)} style={{ width: `${Math.min(percentage, 100)}%` }} />
        </div>
    );

    const UsageCard = ({ title, subtitle, percentage, resetTime, isPrimary = false }: { title: string; subtitle: string; percentage: number; resetTime?: string; isPrimary?: boolean }) => {
        const status = getStatusInfo(percentage);
        const StatusIcon = status.icon;

        return (
            <div className={cn("rounded-xl border bg-card/50 p-4", isPrimary ? "border-border/60 shadow-sm" : "border-border/40")}>
                <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className={cn("font-mono font-bold", status.color, isPrimary ? "text-base" : "text-sm")}>{Math.round(percentage)}%</span>
                    </div>
                </div>
    ...
    // Header Button
    const maxPercentage = usage ? Math.max(usage.sessionPercentage || 0, usage.weeklyPercentage || 0) : 0;
        ...
        <Button variant="ghost" size="sm" className="h-9 gap-3 bg-secondary border border-border px-3">
            ...
            {hasKey && (
                <div className="h-1.5 w-16 bg-muted-foreground/20 rounded-full overflow-hidden">
                    <div className={cn("h-full transition-all duration-500", getProgressBarColor(maxPercentage))} style={{ width: `${Math.min(maxPercentage, 100)}%` }} />
                </div>
            )}
        </Button>
    );
            ...
            <PopoverContent className="w-80 p-0 overflow-hidden bg-background/95 backdrop-blur-xl border-border shadow-2xl" align="end" sideOffset={8}>
                ...
                <div className="p-4 space-y-4">
                    {error ? (
                        ...
                        <>
                            {/* Primary Card */}
                            <UsageCard title="Session Usage" subtitle="5-hour rolling window" percentage={usage.sessionPercentage} resetTime={usage.sessionResetTime} isPrimary={true} />

                            {/* Secondary Cards Grid */}
                            <div className={cn("grid gap-3", (usage?.costLimit && usage.costLimit > 0) || (usage?.opusWeeklyTokensUsed && usage.opusWeeklyTokensUsed > 0) ? "grid-cols-2" : "grid-cols-1")}>
                                <UsageCard title="Weekly" subtitle="All models" percentage={usage.weeklyPercentage} resetTime={usage.weeklyResetTime} />
                            ...
                            {/* Extra Usage / Cost */}
                            {usage.costLimit && usage.costLimit > 0 && <UsageCard title="Extra Usage" subtitle={`${usage.costUsed} / ${usage.costLimit} ${usage.costCurrency}`} percentage={((usage.costUsed || 0) / usage.costLimit) * 100} />}
                        </>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between px-4 py-2 bg-secondary/10 border-t border-border/50">
                    <a href="https://status.claude.com" target="_blank" rel="noreferrer" className="text-[10px] text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors">
